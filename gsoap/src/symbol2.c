...
{	Entry *p;
	...
	{ fprintf(fmsg, "Saving %s client calling stubs\n", pathsoapClient);
	  ...
	  { flib=fopen(pathsoapClientLib,"w");
	    ...
	    fprintf(fmsg, "Saving %s client calling stubs with static serializers for libs\n", pathsoapClientLib);
	...
	{ fprintf(fmsg, "Saving %s server request dispatcher\n", pathsoapServer);
	  ...
	  { flib=fopen(pathsoapServerLib,"w");
	    ...
	    fprintf(fmsg, "Saving %s server request dispatcher with static serializers for libs\n", pathsoapServerLib);
	    fprintf(flib, "\n#ifndef WITH_NOGLOBAL\n#define WITH_NOGLOBAL\n#endif");
	    fprintf(flib, "\n#define SOAP_FMAC3 static");
	    fprintf(flib, "\n#include \"%s\"", soapC);
...
{ Symbol *p, *q;
  ...
  { if (*p->name != '~')
	...
	s++;
      n = (int)(strlen(s) - 2);
      for (i = 1; i < n; i++)
      { if ((s[i-1] != '_' && s[i] == '_' && s[i+1] == '_' && s[i+2] && s[i+2] != '_') || s[i] == ':')
        { if (s[i+1] == ':')
	  { i++;
	    continue;
	  }
  ...
  { for (sp2 = services; sp2; sp2 = sp2->next)
    ...
    { fprintf(fd, "  <import namespace=\"%s\"", sp2->URI);
      if (sp2->import)
        ...
        fprintf(fd, " schemaLocation=\"%s\"", sp2->URI);
      fprintf(fd, "/>\n");
...
{ char *s, *t;
	...
	{ if (s) s++; else s = t;
	  if (((Tnode*)q->next->info.typ->ref)->type == Tpointer)
            if (q->info.maxOccurs == 1)
	      fprintf(fd, "     <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"unbounded\" nillable=\"true\"/>\n", s, wsdl_type(q->next->info.typ->ref, ns1), q->info.minOccurs);
            else
	      fprintf(fd, "     <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"%ld\" nillable=\"true\"/>\n", s, wsdl_type(q->next->info.typ->ref, ns1), q->info.minOccurs, q->info.maxOccurs);
          else
            if (q->info.maxOccurs == 1)
...
{ Service *sp;
  ...
  char *style, *encoding;
  char *xtag, *xtyp;
  char *action = NULL, *method_encoding = NULL, *method_response_encoding = NULL;
  Table *params;
...
int
is_stdqname(Tnode *p)
{ if (p->sym && is_stdstring(p) && (is_eq(p->sym->name, "xsd__QName") || is_eq(p->sym->name, "QName")))
    return 1;
  return p->id && is_stdstring(p) && (is_eq(p->id->name, "xsd__QName") || is_eq(p->id->name, "QName"));
}
